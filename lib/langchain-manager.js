...
const FlowCreationDetector = require('./flow-creation-detector');
...
class LangChainManager {
    constructor(memoryManager, mcpClient, language = 'zh-CN') {
        ...
        // 初始化流程创建检测器
        this.flowCreationDetector = new FlowCreationDetector(this);
    ...
    loadScenarios() {
        try {
            ...
            this.scenarios = JSON.parse(scenariosData).scenarios;
    ...
    async executeScenarioChat(scenario, message, llmConfig, sessionId, dynamicData = {}) {
        ...
        let detectionResult = { isFlowCreation: false, confidence: 0 }; // 默认值
        
        try {
            detectionResult = this.flowCreationDetector.detectFlowCreationIntent(message);
            // console.log('🔍 流程创建意图检测结果:', detectionResult);
            
            // 应用回退机制
            const fallbackResult = this.flowCreationDetector.handleFallback(message, detectionResult);
            
            if (fallbackResult.shouldFallback) {
                // console.log('🔄 触发回退机制:', fallbackResult.fallbackReason);
                processedMessage = fallbackResult.processedPrompt;
                fallbackInfo = fallbackResult;
            } else if (fallbackResult.needsMoreInfo) {
                // console.log('ℹ️ 需要更多信息引导用户');
                processedMessage = fallbackResult.processedPrompt;
                // 可以在这里添加引导消息到响应中
            } else {
                // 正常处理情况（置信度 >= 0.6 或其他情况）
                processedMessage = fallbackResult.processedPrompt;
                
                if (detectionResult.isFlowCreation) {
                    // console.log('🎯 检测到流程创建意图，置信度:', detectionResult.confidence);
                    // console.log('🏷️ 检测到的场景:', detectionResult.detectedScenarios);
                    
                    // 参数收集检查
                    const paramCheck = this.flowCreationDetector.collectParameters(message, detectionResult);
                    if (!paramCheck.isComplete && detectionResult.confidence > 0.7) {
                        // console.log('⚠️ 检测到缺少参数:', paramCheck.missingParams);
                    }
                }
            }
        } catch (error) {
            // console.error('❌ 流程创建意图检测失败:', error);
            // 使用错误恢复机制
            const errorRecovery = this.flowCreationDetector.handleError(error, message);
            processedMessage = errorRecovery.processedPrompt;
            fallbackInfo = errorRecovery;
            // 保持默认的 detectionResult 值
        }
            ...
            const enrichedDynamicData = {
                ...dynamicData,
                lang: this.language,
                flowCreationDetected: detectionResult.isFlowCreation,
            ...
            // Save to memory
            this.memoryManager.addConversation(sessionId, 'user', processedMessage, scenario);
            ...
            this.memoryManager.addToSessionContext(sessionId, { role: 'user', content: processedMessage });
    ...
    detectToolTrigger(message) {
        ...
        console.log('[detectToolTrigger] Match result:', match);
        if (match) {
    ...
    async executeScenarioChatStream(scenario, message, llmConfig, sessionId, dynamicData = {}, onChunk = null) {
        ...
        try {
            detectionResult = this.flowCreationDetector.detectFlowCreationIntent(message);
            // console.log('🔍 流程创建意图检测结果（流式）:', detectionResult);
            
            // 应用回退机制
            const fallbackResult = this.flowCreationDetector.handleFallback(message, detectionResult);
            
            if (fallbackResult.shouldFallback) {
                // console.log('🔄 触发回退机制（流式）:', fallbackResult.fallbackReason);
                processedMessage = fallbackResult.processedPrompt;
                fallbackInfo = fallbackResult;
            } else if (fallbackResult.needsMoreInfo) {
                // console.log('ℹ️ 需要更多信息引导用户（流式）');
                processedMessage = fallbackResult.processedPrompt;
            } else {
                // 正常处理情况（置信度 >= 0.6 或其他情况）
                processedMessage = fallbackResult.processedPrompt;
                ...
                if (detectionResult.isFlowCreation) {
                    // console.log('🎯 检测到流程创建意图（流式），置信度:', detectionResult.confidence);
                    // console.log('🏷️ 检测到的场景:', detectionResult.detectedScenarios);
                }
            }
        } catch (error) {
            // console.error('❌ 流程创建意图检测失败（流式）:', error);
            // 使用错误恢复机制
            const errorRecovery = this.flowCreationDetector.handleError(error, message);
            processedMessage = errorRecovery.processedPrompt;
            fallbackInfo = errorRecovery;
            ...
            if (!toolTrigger.isToolTrigger) {
                toolTrigger = this.shouldForceToolMode(scenario, processedMessage);
            ...
            if (!toolTrigger.isToolTrigger) {
                ...
                return await this.executePureLLMChatStream(scenario, processedMessage, llmConfig, sessionId, enrichedDynamicData, onChunk);
            }
            
            // Tool call mode
            // console.log('🔧 Using tool call mode to process message');
            
            // Send start event only if not in direct execution mode
            // In direct execution mode, we'll send tool_call event directly without creating empty message bubble
            if (!toolTrigger.directExecution && onChunk && typeof onChunk === 'function') {
            ...
            if (toolTrigger.directExecution) {
                ...
                try {
                    ...
                    // Generate different prompts based on tool type
                    let explanationPrompt;
                    switch (toolTrigger.toolName) {
                        case 'get-flow':
                            explanationPrompt = `User request: ${toolTrigger.originalMessage}\n\nTool execution result:\n${toolResult}\n\nAs a professional Node-RED system administrator, please analyze the above flow data in detail and explain in natural language:\n1. The overall functionality and purpose of the flow\n2. Main node types and their roles\n3. Connection relationships and data flow between nodes\n4. Working principles and execution logic of the flow\n\nPlease answer in ${responseLanguage} with professional but easy-to-understand language.`;
                            break;
                            
                        case 'get-node-info':
                            explanationPrompt = `User request: ${toolTrigger.originalMessage}\n\nTool execution result:\n${toolResult}\n\nAs a professional Node-RED system administrator, \n\n1：请分析当前选中的每个节点的json代码： \n按照以下结构分别输出每个节点的分析结果： \n 用表格方式输出： \n节点名称，节点类型，类型说明，输入，输出，配置项及其解释 \n\n如果是function类型的要特别描述其代码的功能逐行用注释描述：格式如： \n1:<code>if (msg.payload === '启动') {</code>：检查输入的消息负载是否为字符串"启动"。 \n\n\nPlease answer in ${responseLanguage} with professional but easy-to-understand language.`;
                            break;
                            
                        case 'get-settings':
                            explanationPrompt = `User request: ${toolTrigger.originalMessage}\n\nTool execution result:\n${toolResult}\n\nAs a professional Node-RED system administrator, please:\n1. Analyze the Node-RED configuration JSON returned by the current tool\n2. Describe each configuration key, value, and meaning in table format\n\nPlease answer in ${responseLanguage} with professional but easy-to-understand language.`;
                            break;
                            
                        case 'get-diagnostics':
                            explanationPrompt = `User request: ${toolTrigger.originalMessage}\n\nTool execution result:\n${toolResult}\n\nAs a professional Node-RED system administrator, \n\n1：请分析当前工具返回的Node-red的当前诊断的json \n\n2：用表格方式描述每一项的key和value以及含义 \n3：表格后增加当前资源情况，描述CPU和内存的使用率，并分析当前健康状况 \n\nPlease answer in ${responseLanguage} with professional but easy-to-understand language.`;
                            break;
                            
                        default:
                            // Fallback to original prompt for unknown tools
                            explanationPrompt = `User request: ${toolTrigger.originalMessage}\n\nTool execution result:\n${toolResult}\n\nAs a professional Node-RED system administrator, please analyze the above data and provide a detailed explanation.\n\nPlease answer in ${responseLanguage} with professional but easy-to-understand language.`;
                    }

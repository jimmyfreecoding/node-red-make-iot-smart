        ...
        // console.log('🔧 LLM configuration:', llmConfig);
            ...
            if (toolTrigger.directExecution) {
                ...
                try {
                    ...
                    // Send tool execution result event (remove tool_call event to avoid showing bubble box)
                    if (onChunk && typeof onChunk === 'function') {
                        onChunk({
                            type: 'tool_result',
                            content: toolResult
            ...
            for await (const chunk of stream) {
                ...
                if (chunk.event === 'on_chat_model_stream' && chunk.data?.chunk?.content) {
                    ...
                    // console.log('🔧 Agent action:', chunk.data);
                    if (onChunk) {
                        // console.log('📤 Sending tool_call event');
                        onChunk({
                            type: 'tool_call',
                    ...
                    // console.log('🔧 Tool ended:', chunk.data);
                    // console.log('🔧 Tool raw output:', JSON.stringify(chunk.data?.output, null, 2));
                    ...
                    if (onChunk) {
                        // console.log('📤 Sending tool_result event');
                        // Format tool result

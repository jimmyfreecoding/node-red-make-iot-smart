...
class LangChainManager {
    ...
    detectToolTrigger(message) {
        ...
        // 检测包含get-flow工具调用指示的消息（支持中英文）
        if (message.includes('get-flow') && 
            (message.includes('流程ID为') || message.includes('flow ID'))) {
            console.log('[detectToolTrigger] 检测到get-flow工具调用指示');
            
            // 提取流程ID（支持中英文格式）
            let flowId = '';
            const flowIdMatch = message.match(/(?:流程ID为|flow ID)[\s"']*([^"'\s,，]+)["']?/); 
            if (flowIdMatch) {
        ...
        // 检测包含get-node-info工具调用指示的消息（支持中英文）
        if (message.includes('get-node-info') && 
            (message.includes('节点ID为') || message.includes('node ID'))) {
            console.log('[detectToolTrigger] 检测到get-node-info工具调用指示');
            
            // 提取节点ID列表（支持中英文格式）
            let nodeIds = '';
            const nodeIdMatch = message.match(/(?:节点ID为|node IDs?)\s*\[([^\]]+)\]/); 
            if (nodeIdMatch) {
            ...
            for await (const chunk of stream) {
                ...
                if (onChunk && typeof onChunk === 'function') {
                    ...
                    if (shouldContinue === false) {
                        console.log('🛑 在纯LLM流式块处理前收到停止信号，中断处理');
                        break;
                    }
                }
                ...
                if (content) {
                    ...
                    if (onChunk && typeof onChunk === 'function') {
                        ...
                        // 如果onChunk返回false，停止流式处理
                        if (shouldContinue === false) {
                            console.log('🛑 收到停止信号，中断纯LLM流式处理');
                            break;
                        }
                    }
                }
        ...
        try {
            ...
            for await (const chunk of stream) {
                ...
                if (onChunk) {
                    const shouldContinue = onChunk({ type: 'heartbeat' });
                    if (shouldContinue === false) {
                        console.log('🛑 在流式事件处理前收到停止信号，中断处理');
                        break;
                    }
                }
